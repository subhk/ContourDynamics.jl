...
function _sqg_rankine_energy(R, jump, delta)
    # Independent Fourier-Bessel evaluation of
    # H = 1/2 ∫ θ Λ_delta^(-1) θ dA, where the softened Green function has
    # Fourier multiplier exp(-delta*|k|)/|k|.
    integrand(k) = exp(-delta * k) * (besselj1(R * k) / k)^2
    cutoff = 30 / delta
    return π * jump^2 * R^2 * _sqg_gl5_integral(integrand, 0.0, cutoff, 400)
end

function _sqg_rankine_speed(R, jump, delta)
    integrand(phi) = cos(phi) / sqrt(2R^2 * (1 - cos(phi)) + delta^2)
...
function _periodic_sqg_fourier_energy(domain, contours, delta, modes)
    ...
    for m in -modes:modes, n in -modes:modes
        ...
        theta_hat = sum(c.pv * _sqg_polygon_fourier_integral(c, kx, ky)
                        for c in contours)
        result += exp(-delta * k) / k * abs2(theta_hat) / (2 * area)
    end
    return result
end
...
@testset "Surface quasi-geostrophic verification" begin
    @testset "regularized Rankine patch" begin
        R, jump, delta = 0.8, 1.2, 0.3
        exact_energy = _sqg_rankine_energy(R, jump, delta)
        exact_speed = _sqg_rankine_speed(R, jump, delta)
        energy_errors = Float64[]

        for n in (64, 128)
            contour = circular_patch(R, n, jump)
            prob = ContourProblem(SQGKernel(delta), UnboundedDomain(), [contour])
        ...
        prob = ContourProblem(SQGKernel(delta), UnboundedDomain(),
    ...
    @testset "periodic energy matches independent Fourier inversion" begin
        ...
        clear_ewald_cache!()
        setup_ewald_cache!(domain, SQGKernel(delta); n_fourier=12, n_images=12)
        prob = ContourProblem(SQGKernel(delta), domain, deepcopy(contours))
        reference = _periodic_sqg_fourier_energy(domain, contours, delta, 64)
