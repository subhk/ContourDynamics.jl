...
function _problem_test_cuda_loaded()
    try
        @eval using CUDA
        return true
    catch
...
@testset "Problem API" begin
    ...
    @testset "Problem factory — device" begin
        ...
        if _problem_test_cuda_loaded()
            prob_gpu = Problem(; contours=[c], dt=0.01, dev=GPU())
            @test prob_gpu.contour_problem.dev === GPU()
            @test !(prob_gpu.stepper.k1 isa Vector)
        else
            # The high-level Problem constructor must allocate timestep buffers on
            # the selected device. Without CUDA loaded, GPU construction fails early
            # instead of creating CPU-backed steppers.
            @test_throws ErrorException Problem(; contours=[c], dt=0.01, dev=GPU())
            @test_throws ErrorException Problem(; kernel=:qg, Ld=1.0, contours=[c], dt=0.01, dev=GPU())
