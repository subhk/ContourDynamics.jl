...
@testset "Edge Cases & Error Handling" begin
    ...
    @testset "Invalid domain parameters" begin
        ...
        @test_throws ArgumentError ContourProblem(EulerKernel(), domain64, [c32])

        Ld = SVector(1.0f0)
        F = 1.0f0 / (2.0f0 * Ld[1]^2)
        coupling = SMatrix{2,2,Float32}(-F, F, F, -F)
        kernel = MultiLayerQGKernel(Ld, coupling)
        layers = ([c32], [circular_patch(0.25, 8, -0.5f0; T=Float32)])
        @test_throws ArgumentError MultiLayerContourProblem(kernel, domain64, layers)
    end
