...
const _GPU_ODE_MSG = "OrdinaryDiffEq integration uses a CPU vector state and mutates live CPU contours. Construct the problem with dev=CPU(), or use ContourDynamics timesteppers for GPU() problems."
...
# unsafe with step rejection: a rejected trial step's mutations to CPU contours
...
!!! warning "Solver compatibility"
    This bridge is CPU-only because the RHS closure mutates live CPU contours
